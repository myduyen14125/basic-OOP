...
template <typename T>
T &Vecto<T>::operator [] (int i) const throw (out_of_range){
    if (i >= n || i < 0) {
...
{
    if (this->n != a.n) {
        throw invalid_argument("Different size, can't add!");
    }
    else {
        Vecto<T> b = *this;
        for(int i = 0; i < b.n; i++)
...
{
    if (this->n != a.n) {
        throw invalid_argument("Different size, can't subtract!");
    }
    else {
        Vecto<T> b = *this;
        for(int i = 0; i < b.n; i++)
...
{
    if (this->n != a.n) {
        throw invalid_argument("Different size, can't mul!");
    }
    else {
        T p = 0;
        for (int i = 0; i < n; i++)
